...
export class StockRepository {
  ...
  async getStock(productId: string): Promise<Stock | null> {
    const [rows] = await this.db.query("SELECT * FROM stock WHERE product_id = ?", [productId]);
  ...
  async updateStock(productId: string, quantity: number): Promise<void> {
    await this.db.query("UPDATE stock SET quantity = quantity + ? WHERE product_id = ?", [quantity, productId]);
    await this.db.query("INSERT INTO stock_movements (product_id, type, quantity, reason) VALUES (?, ?, ?, ?)", [
      productId,
      quantity > 0 ? "IN" : "OUT",
      Math.abs(quantity),
      "Manual Adjustment",
