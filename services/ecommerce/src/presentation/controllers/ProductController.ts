...
export class ProductController {
    ...
    updateProduct = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            if (product.merchantId !== req.user.userId && req.user.role !== 'admin') {
                res.status(403).json({ success: false, error: { code: 'FORBIDDEN', message: 'Not authorized to update this product' } });
                return;
            }
    ...
    deleteProduct = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            if (product.merchantId !== req.user.userId && req.user.role !== 'admin') {
                res.status(403).json({ success: false, error: { code: 'FORBIDDEN', message: 'Not authorized to delete this product' } });
                return;
            }
