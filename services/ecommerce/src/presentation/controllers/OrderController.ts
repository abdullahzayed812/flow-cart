...
export class OrderController {
  constructor(private checkoutUseCase: CheckoutUseCase, private orderRepository: IOrderRepository) {}
  ...
  checkout = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!shippingAddress) {
        res
          .status(400)
          .json({ success: false, error: { code: "VALIDATION_ERROR", message: "Shipping address is required" } });
        return;
      }
  ...
  getOrder = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (order.userId !== req.user.userId && req.user.role !== "admin" && req.user.role !== "merchant") {
        res
          .status(403)
          .json({ success: false, error: { code: "FORBIDDEN", message: "Not authorized to view this order" } });
        return;
      }
  ...
  cancelOrder = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (order.userId !== req.user.userId) {
        res
          .status(403)
          .json({ success: false, error: { code: "FORBIDDEN", message: "Not authorized to cancel this order" } });
        return;
      }
