...
export class AddToCartUseCase {
  constructor(private cartRepository: ICartRepository, private warehouseClient: WarehouseClient) {}

  async execute(dto: AddToCartDTO): Promise<CartItem> {
    // Verify product exists via Warehouse Service
    const product = await this.warehouseClient.getProduct(dto.productId);
    if (!product) {
